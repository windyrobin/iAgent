...
//  print('total request count : ' + gReqCount);
//  print('total socket  count : ' + gSocketCount);
//}, 10000);
...
function Agent(options) {
  ...
  self.maxRequestPerConnection  = self.options.maxRequestPerConnection || 1000;

  self.on('free', function(socket, host, port) {

    var name = host + ':' + port;
    var sn = self.requests[name];
    if (sn && sn.length) {
      var req = sn.shift();
      ...
      if((sn && sn.length >= self.maxSockets) || 
        (socket._reqCount >= self.maxRequestPerConnection)){
